...
class NotificationService {
    ...
    static async createNotification(userId, message, type, userModel) {
        try {
            console.log('Creating notification:', { userId, message, type, userModel });
    ...
    static async notifyAppointmentBooked(mentorId, learnerName, dateTime, skill) {
        return this.createNotification(
            mentorId,
            `New appointment request from ${learnerName} for ${skill} on ${new Date(dateTime).toLocaleString()}`,
    ...
    static async notifyAppointmentAccepted(learnerId, mentorName, dateTime, skill) {
        return this.createNotification(
            learnerId,
            `Your appointment with ${mentorName} for ${skill} on ${new Date(dateTime).toLocaleString()} has been accepted`,
