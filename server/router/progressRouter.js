...
const ensureMentor = (req, res, next) => {
  if (req.userType !== 'mentor') {
    return res.status(403).json({ error: "Access denied. Mentor only endpoint." });
  }
  next();
};
...
const ensureLearner = (req, res, next) => {
  if (req.userType !== 'learner') {
    return res.status(403).json({ error: "Access denied. Learner only endpoint." });
  }
  next();
};

// 1. Learning Path Management
router.post('/api/learner/learning-path', [authMiddleware, ensureLearner], async (req, res) => {
  try {
    const { selectedCategory, selectedSkills } = req.body;
    
    let learningPath = await LearningPath.findOne({ learnerId: req.learner._id });
...
router.get('/api/learner/learning-path/:learnerId', [authMiddleware, ensureLearner], async (req, res) => {
  try {
    const learningPath = await LearningPath.findOne({ learnerId: req.params.learnerId });
    ...
    const progress = await Progress.findOne({ learnerId: req.params.learnerId });
    
    if (progress) {
      // Update learning path skills with actual progress
      learningPath.selectedSkills = learningPath.selectedSkills.map(skill => {
        const skillProgress = progress.skillProgress.find(sp => sp.skillName === skill.name);
        if (skillProgress) {
...
router.post('/api/learner/skill-progress', [authMiddleware, ensureLearner], async (req, res) => {
  try {
    const { skillName, progress } = req.body;
    
    const learningPath = await LearningPath.findOne({ learnerId: req.learner._id });
    
    if (!learningPath) {
      return res.status(404).json({ error: 'Learning path not found' });
    }
    
    const skillIndex = learningPath.selectedSkills.findIndex(s => s.name === skillName);
    
    if (skillIndex === -1) {
      return res.status(400).json({ error: 'Skill not found in learning path' });
    }
...
// 3. Course Progress
router.get('/api/mentor/course-progress/:courseId', [authMiddleware, ensureMentor], async (req, res) => {
  try {
...
router.post('/api/mentor/course-progress', [authMiddleware, ensureMentor], async (req, res) => {
  try {
...
// 4. Assignments
router.post('/api/mentor/assignment', [authMiddleware, ensureMentor], async (req, res) => {
  try {
    const { courseId, title, description, dueDate } = req.body;
    
    const progress = await Progress.findOne({ courseId, mentorId: req.mentor._id });
...
router.put('/api/mentor/assignment/:assignmentId', [authMiddleware, ensureMentor], async (req, res) => {
  try {
...
// 5. Sessions
router.post('/api/mentor/session', [authMiddleware, ensureMentor], async (req, res) => {
  try {
...
router.put('/api/mentor/session/:sessionId', [authMiddleware, ensureMentor], async (req, res) => {
  try {
...
router.patch('/api/skill-category/:categoryName/skills', async (req, res) => {
  try {
    ...
    const existingSkillNames = new Set(category.skills.map(skill => skill.name));
    const newSkillNames = new Set(skills.map(skill => skill.name));
    const duplicates = [...newSkillNames].filter(name => existingSkillNames.has(name));
...
router.get('/api/mentor/mentees-progress', [authMiddleware, ensureMentor], async (req, res) => {
  try {
    ...
    for (const learningPath of learningPaths) {
      ...
      const progress = progressRecords.find(
        p => p.learnerId._id.toString() === learningPath.learnerId._id.toString()
      );

      // Filter skills to only those taught by this mentor
      const relevantSkills = learningPath.selectedSkills.filter(
        skill => mentorProfile.skills.includes(skill.name)
      );
      ...
      const skillsProgress = relevantSkills.map(skill => {
        ...
        const completedAssignments = skillProgress?.completedAssignments || [];
        const completedSkillAssignments = skillAssignments.filter(
          sa => sa.skillName === skill.name && completedAssignments.includes(sa._id)
        );
      ...
      menteeProgress.push({
        learner: learningPath.learnerId,
        overallProgress,
    ...
    res.status(200).json({
      menteeProgress: menteeProgress.sort((a, b) => b.overallProgress - a.overallProgress)
    });
  } catch (error) {
...
router.post('/api/mentor/skill/create-assignment', [authMiddleware, ensureMentor], async (req, res) => {
  try {
    const {
      skillName,
      title,
    ...
    const mentorProfile = await MentorProfile.findOne({ mentorId: req.mentor._id });
...
router.get('/api/skill-assignments/:skillName', [authMiddleware], async (req, res) => {
  try {
    const { skillName } = req.params;
    
    const assignments = await SkillAssignment.find({ skillName })
...
router.post('/api/learner/complete-assignment/:assignmentId', [authMiddleware, ensureLearner], async (req, res) => {
  try {
    ...
    let skillProgress = progress.skillProgress.find(s => s.skillName === assignment.skillName);
    if (!skillProgress) {
    ...
    if (skillProgress.completedAssignments.some(id => id.toString() === assignmentId)) {
    ...
    await progress.save();
    console.log('Progress saved, recalculated progress:', progress.skillProgress.find(s => s.skillName === assignment.skillName).progress);

    // Update learning path progress
    const learningPath = await LearningPath.findOne({ learnerId: req.learner._id });
    if (learningPath) {
      const skillIndex = learningPath.selectedSkills.findIndex(s => s.name === assignment.skillName);
      if (skillIndex !== -1) {
        const updatedProgress = progress.skillProgress.find(s => s.skillName === assignment.skillName).progress;
        ...
        console.log('Skill not found in learning path:', assignment.skillName);
      }
    } else {
    ...
    const updatedSkillProgress = progress.skillProgress.find(s => s.skillName === assignment.skillName);

    res.status(200).json({
      message: 'Assignment marked as completed',
      skillProgress: updatedSkillProgress,
      overallProgress: progress.overallProgress,
      learningPathProgress: learningPath ? learningPath.completedProgress : null
    });
  } catch (error) {
...
router.get('/api/mentor/created-assignments', [authMiddleware, ensureMentor], async (req, res) => {
  try {
    // Get all assignments created by the mentor
    const assignments = await SkillAssignment.find({ createdBy: req.mentor._id })
      .sort({ createdAt: -1 });
...
router.get('/api/learner/completed-assignments', [authMiddleware, ensureLearner], async (req, res) => {
  try {
...
router.get('/api/learner/completed-assignments/:skillName', [authMiddleware, ensureLearner], async (req, res) => {
  try {
    const { skillName } = req.params;
    const progress = await Progress.findOne({ learnerId: req.learner._id });
    ...
    const skillProgress = progress.skillProgress.find(s => s.skillName === skillName);
    if (!skillProgress) {
      return res.status(200).json({ completedAssignments: [] });
    }
...
router.get('/api/learner/skill-assignments-status/:skillName', [authMiddleware, ensureLearner], async (req, res) => {
  try {
    const { skillName } = req.params;
    ...
    const progress = await Progress.findOne({ learnerId: req.learner._id });
    const skillProgress = progress?.skillProgress.find(s => s.skillName === skillName);
    const completedIds = skillProgress ? skillProgress.completedAssignments.map(id => id.toString()) : [];
    
    // Mark assignments as completed or pending
    const assignmentsWithStatus = allAssignments.map(assignment => ({
      ...assignment.toObject(),
      status: completedIds.includes(assignment._id.toString()) ? 'completed' : 'pending'
    }));
...
router.get('/api/mentor/mentee-profile/:learnerId', [authMiddleware, ensureMentor], async (req, res) => {
  try {
    const { learnerId } = req.params;

    // Get learner profile
    const learnerProfile = await LearnerProfile.findOne({ learnerId })
      .populate('learnerId', 'firstName lastName email');
    ...
    const response = {
      profile: {
        ...
        fullName: learnerProfile.fullName || `${learnerProfile.learnerId.firstName} ${learnerProfile.learnerId.lastName}`,
      ...
      upcomingSessions: upcomingAppointments.map(appointment => {
        ...
        return {
          title: appointment.mentorId ? 
            `Session with ${appointment.mentorId.firstName || ''} ${appointment.mentorId.lastName || ''}`.trim() : 
            'Session',
