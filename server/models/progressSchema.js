...
progressSchema.methods.calculateSkillProgress = async function (skillName) {
  ...
  const totalWeightage = allAssignments.reduce(
    (sum, assignment) => sum + assignment.weightage,
    0
  );
  const completedWeightage = completedAssignments.reduce(
    (sum, assignment) => sum + assignment.weightage,
    0
  );
...
progressSchema.pre("save", async function (next) {
  try {
    ...
    const updatePromises = this.skillProgress.map(
      async (skillProgress, index) => {
        const skillPath = `skillProgress.${index}.completedAssignments`;
        if (modifiedPaths.includes(skillPath)) {
          const newProgress = await this.calculateSkillProgress(
...
const SkillAssignment = mongoose.model(
  "SkillAssignment",
