...
const MentorProfile = () => {
  ...
  useEffect(() => {
    const getMentorProfile = async () => {
      ...
      try {
        // Fetch mentor profile directly from the mentor profile API
        ...
        if (data) {
          setMentorData({
            ...
            image: data.profilePictureUrl
              ? `${API_ENDPOINTS.BASE_URL}/uploads/${data.profilePictureUrl}`
              : null,
          });
        }
      } catch (error) {
            ...
            <div className="flex flex-col md:flex-row gap-8 items-start">
                ...
                <div className="relative group">
                  ...
                  <div className="relative">
                    <img
                      ...
                      className="w-44 h-44 rounded-full object-cover border-4 border-white/10"
                      onError={(e) => {
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  <h1 className="text-3xl font-bold text-white">
                    {mentorData?.fullName || mentorData?.name}
                  </h1>
                  {mentorData?.isVerified ? (
                ...
                <p className="text-primary-color text-lg mb-4">
                  {mentorData?.expertise || mentorData?.role}
                </p>
                ...
                <div className="flex flex-wrap gap-3">
                  {(mentorData?.skills || mentorData?.topSkills || []).map(
                    (skill, index) => (
                      <span
                        key={index}
              ...
              <div className="flex flex-col gap-3 min-w-[200px]">
                ...
                <Link
                  to={
                    mentorData?.isVerified
                      ? `/mentee/book?id=${searchParams.get("id")}`
                      : "#"
                  }
                  className={`w-full ${
                    !mentorData?.isVerified && "pointer-events-none"
                  }`}
                >
                  <motion.button
          ...
          {activeTab === "experience" && (
            <div className="md:col-span-3 bg-white/5 backdrop-blur-xl rounded-xl border border-white/10 p-6">
              ...
              <div className="space-y-6">
                {(mentorData?.workExperiences || mentorData?.experience || [])
                  .length > 0 ? (
                  (mentorData?.workExperiences || mentorData?.experience).map(
                    (exp, index) => (
                      <div
                        key={index}
                      ...
                      >
                        ...
                        <p className="text-primary-color/90">
                ...
                {Object.entries(mentorData?.availability || {})
                  ...
                  .map(([day, slots]) => (
                    <div key={day} className="bg-white/5 rounded-lg p-4">
                      <h3 className="text-lg font-medium text-white capitalize mb-3">
                        {day}
                      </h3>
                      {slots?.length > 0 ? (
                        <div className="flex flex-wrap gap-2">
